    ...
    <main className="min-h-screen bg-[#0D0D1A]">
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="flex flex-wrap gap-4 justify-center">
          <Link href={`/${locale}/categories/construction`} className="px-6 py-3 bg-[#1A1A2E] text-white rounded-full hover:bg-[#25253a] transition-colors">
            🔨 Строителство
          </Link>
          <Link href={`/${locale}/categories/home`} className="px-6 py-3 bg-[#1A1A2E] text-white rounded-full hover:bg-[#25253a] transition-colors">
            🧹 Домашни услуги
          </Link>
          <Link href={`/${locale}/categories/beauty`} className="px-6 py-3 bg-[#1A1A2E] text-white rounded-full hover:bg-[#25253a] transition-colors">
            💅 Красота
          </Link>
          <Link href={`/${locale}/categories/photography`} className="px-6 py-3 bg-[#1A1A2E] text-white rounded-full hover:bg-[#25253a] transition-colors">
            📸 Фотография
          </Link>
          <Link href={`/${locale}/specialists`} className="px-6 py-3 bg-[#1DB954] text-white rounded-full hover:bg-[#169b43] transition-colors">
            Всички категории →
          </Link>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((category) => (
            <ProZonaCategoryCard key={category.id} data={category} />
          ))}
        </div>
      </section>

      {/* Популярни специалисти */}
      <section className="container mx-auto px-4 py-12">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-3xl font-bold text-white">Популярни специалисти</h2>
          <Link href={`/${locale}/specialists`} className="text-[#1DB954] hover:underline text-lg">
            Виж всички →
          </Link>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="bg-gradient-to-r from-[#1DB954] to-[#169b43] rounded-2xl p-12 text-center">
          ...
          <p className="text-xl text-white mb-8">
            Присъедини се към нас и намери нови клиенти
          </p>
          <Link
            href={`/${locale}/for-specialists`}
            className="inline-block px-8 py-4 bg-white text-[#1DB954] font-semibold rounded-lg hover:bg-gray-100 transition-colors text-lg"
          >
            Започни да печелиш
          </Link>
        </div>
