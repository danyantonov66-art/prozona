...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Вземи ролята директно от API за да избегнем stale session
      ...
      window.location.href = `/${locale}/admin`
       ...
       window.location.href = `/${locale}/specialist/dashboard`
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#0D0D1A] flex items-center justify-center py-12 px-4">
      <div className="max-w-md w-full bg-[#1A1A2E] rounded-lg p-8">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-white">Вход в ProZona</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 bg-[#1DB954] text-white rounded-lg font-medium hover:bg-[#169b43] disabled:opacity-50"
          >
        ...
        <button
          ...
          className="w-full py-3 bg-white text-gray-900 rounded-lg font-medium hover:bg-gray-100 flex items-center justify-center gap-2"
        >
        ...
        <p className="text-center text-gray-400 mt-6">
          Нямате акаунт?{' '}
          <Link href={`/${locale}/register`} className="text-[#1DB954] hover:underline">
            Регистрирайте се
          </Link>
        </p>
      </div>
