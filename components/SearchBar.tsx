...
export default function SearchBar({ locale = "bg" }: SearchBarProps) {
    ...
    for (const category of categories) {
      ...
      for (const sub of category.subcategories) {
        items.push({
          ...
          label: sub.name,
          href: `/${locale}/categories/${category.id}/${sub.id}`,
  ...
  useEffect(() => {
    const loadSpecialists = async () => {
      ...
      try {
        ...
        console.error("SearchBar specialist fetch error:", error)
        setSpecialistResults([])
      } finally {
        setLoading(false)
      }
  ...
  const localResults = useMemo(() => {
    ...
    const startsWithMatches = searchIndex.filter((item) =>
      normalize(item.label).startsWith(q)
    )
