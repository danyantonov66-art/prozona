...
const filePath = path.join('app', 'api', 'admin', 'send-email', 'route.ts');

let content = fs.readFileSync(filePath, 'utf8');
const usesCRLF = content.includes('\r\n');
let normalized = content.replace(/\r\n/g, '\n');

if (normalized.includes('offers_feature_announcement')) {
  console.log('Типът "offers_feature_announcement" вече съществува — нищо не променям.');
  process.exit(0);
}

const anchor = '    return NextResponse.json({ success: true })';

if (!normalized.includes(anchor)) {
  console.error('ГРЕШКА: Не намерих очаквания anchor ред преди "return NextResponse.json({ success: true })".');
  process.exit(1);
}

const newBlock = `    if (type === "offers_feature_announcement") {
...
normalized = normalized.replace(anchor, newBlock);
...
console.log('УСПЕХ: Добавен е новият тип "offers_feature_announcement" в route-а.');
console.log('Бекъп: ' + filePath + '.backup');
